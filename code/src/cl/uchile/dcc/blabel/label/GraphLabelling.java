...
public class GraphLabelling implements Callable<GraphLabellingResult> {
	...
	/** 
	 * Canonicalise HubLabel.graph with standard arguments.
	 * @param data
	 */
	...
	/**
	 * Canonicalise HubLabel.graph with custom arguments (e.g., custom hashing)
	 * @param data
	 * @param cla
	 */
	 ...
	 * and args provided. Spits out a result with a canonical HubLabel.graph.
	 */
	public GraphLabellingResult call() throws InterruptedException, HashCollisionException{
		// first hash the HubLabel.graph
		...
		// more efficient to split the HubLabel.graph
		...
		for(HashGraph bnp:bnps){
			...
			// get the canonical HubLabel.graph for the partition
			GraphResult ghp = gc.getCanonicalGraph();
			
			// get the mapped data
			TreeSet<Node[]> mapped = ghp.getGraph();
			
			// make sure the HubLabel.graph has not been seen before
			...
			// update the bnodes in the original full HubLabel.graph
			hg.updateBNodeHashes(ghp.getHashGraph().getBlankNodeHashes());
		}
		
		
		// will store a unique HubLabel.graph-level hash
		HashCode ghash = null;
		
		if(args.upg){
			// need to mux in a HubLabel.graph-level unique hash
	...
	public static class GraphLabellingArgs{
		...
		// if true, mux with a HubLabel.graph level hash
		 ...
		 * on the level of a given HubLabel.graph or a given partition.
		 * 
		 * If set to false, a unique partition-level hash is muxed with
		 * all blank nodes in each partition.
		 * 
		 * If set to true, a unique HubLabel.graph-level hash is muxed with all
		 * blank nodes in the entire HubLabel.graph.
		 * 
		 * For example take:
		 * _:a :p _:b .
		 * _:b :p _:c .
		 ...
		 * <q> :p <w> .
		 * 
		 * The labelled version of the first HubLabel.graph would be a
		 * subset of the second HubLabel.graph if set to false (since
	...
	public static class GraphLabellingResult{
		 ...
		 * in the arguments. This is a unique HubLabel.graph level hash
		 * used to mux the blank nodes. It serves as an overall
		 * hash for the HubLabel.graph.
		 * 
		 * @param ghash
		 */
		...
		/**
		 * Get the final canonicalised HubLabel.graph (with blank node labels)
		 * @return
		 */
		...
		/**
		 * Get the final hash HubLabel.graph with canonical labels
		 * @return
		 */
		...
		/**
		 * Get the count of blank nodes in the HubLabel.graph
		 * @return
		 */
		...
		/**
		 * Get the number of blank node partitions in the HubLabel.graph
		 * @return
		 */
