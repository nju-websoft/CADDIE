...
public class dbReader {
	...
	/*
	public void dbInit(String HubLabel.database, String table, int u) {
		DBName = HubLabel.database;
		tableName = table;
		String sql;
		try {
			Class.forName("com.mysql.jdbc.Driver");
			conn = DriverManager.getConnection(DB_URL, USER, PASS);
			stmt = conn.createStatement();						
			sql = "CREATE DATABASE IF NOT EXISTS " + HubLabel.database;
			stmt.executeUpdate(sql);			
			// use HubLabel.database
			sql = "USE " + HubLabel.database;
			stmt.executeUpdate(sql);
