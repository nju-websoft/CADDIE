package HubLabel.PLL;

import HubLabel.database.dbReader;
import HubLabel.database.dbWriter;
import HubLabel.graph.Hop;
import HubLabel.graph.OrderGen;
...
public class WeightedPLL {
	...
	int nodeNum;
	long labelSize = 0;
	ArrayList<ArrayList<Hop>> graph; // raw HubLabel.graph edge
...
//		pw.close();
	...
	public void pllDBDeal(String DBName, WeightedGraph ww, String mode) throws IOException{
		...
		if (mode.equals("PLL")) {
			og1.orderGen1(w1, newName, oldName);
			Prune(w1);
			System.out.println("avg HubLabel.PLL: "+(((double)labelSize)/w1.nodeNum + 1));
			orderOutputPLL(w1, DBName);
			labelOutputPLL(w1, DBName);
		}			
		else {
	...
	public void pllIndexDeal2(WeightedGraph ww, String baseFolder) throws IOException {
		...
		og1.orderGen1(ww, newName, oldName); // mode: HubLabel.PLL
//		System.out.println("NodeNum: " + ww.nodeNum);
		Prune(ww);
//		System.out.println("avg CHL: "+(((double)labelSize)/ww.nodeNum));
		System.out.println("avg HubLabel.PLL: "+(((double)labelSize)/ww.nodeNum));
	...
	void orderOutput2PCSG(String baseFolder, WeightedGraph w1) {
		try {
	...
	void orderOutputPLL(WeightedGraph w1, String dbName) {
		...
		for (int i = 0; i < nodeNum; i++)
			d1.insertSubName(w1.reHashTable.get(oldName[i]), i);
		d1.dbWriterEnd();
		System.out.println("HubLabel.PLL Sub Name Generate end!");
		...
		System.out.println("HubLabel.PLL Inverted Table Generate end!");
	}
...
//					d1.insertHubLabel(i, ehv.v, ehv.dis, newName[ehv.par]);
	...
	void labelOutputPLL(WeightedGraph w1, String dbName){
		...
		d1.dbWriterEnd();		
		System.out.println("HubLabel.PLL hub Label Generate end!");
	}
